...
export async function storeProviderCallback(params: {
  ...
  await callbackQueue.add("provider.callback.process", { providerCallbackId: callback.id }, { jobId: `pcb_${callback.id}_${nanoid(6)}`, removeOnComplete: 1000, removeOnFail: 1000 })
